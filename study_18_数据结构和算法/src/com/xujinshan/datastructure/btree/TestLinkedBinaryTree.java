...
public class TestLinkedBinaryTree implements BinaryTree{

	private Node root;//根结点
	//private int size;
	
	public TestLinkedBinaryTree() {
		super();
	}

	public TestLinkedBinaryTree(Node root) {
		super();
		this.root = root;
	}
	...
	public void preOrderTraverse() {		
		if(root != null){
			...
			//2.对左子树进行先序遍历
			//构建一个二叉树，根是左子树的根
			BinaryTree  leftTree = new TestLinkedBinaryTree(root.leftChild);
			leftTree.preOrderTraverse();
			//对右子树进行先序遍历
			//3.构建一个二叉树，根是左子树的根
			BinaryTree  rightTree = new TestLinkedBinaryTree(root.rightChild);
